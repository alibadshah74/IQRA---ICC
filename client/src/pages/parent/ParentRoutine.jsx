...
const ParentRoutine = () => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true)
      try {
        const { data } = await apiClient.get(`/parent/children/${childId}/routine`)
        const res = data?.data ?? data
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-semibold text-slate-900">Children Class Routine</h2>
        <p className="mt-1 text-sm text-slate-500">View daily routines for each child.</p>
        {children.length > 0 && (
          <select
            value={childId}
